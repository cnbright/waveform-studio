...
Waveform Studio 是一个单文件离线示波器波形工作台，用于查看和测量 Tektronix `WFM#003`、Keysight/Agilent `H5` 与 Keysight `OSC` 存储波形文件。
...
- 支持同时导入多个 `.wfm`、`.h5` 与 `.osc` 文件，包括整型 ADC 数据和已换算为伏特的浮点 H5 波形
...
Waveform Studio is a single-file offline oscilloscope waveform workbench for viewing and measuring Tektronix `WFM#003`, Keysight/Agilent `H5`, and Keysight `OSC` waveform files.
...
- Import multiple `.wfm`, `.h5`, and `.osc` files, including integer ADC data and H5 waveforms stored as scaled floating-point volts
